...
def compute_scalp_signal(
    ...
    *,
    ...
    scalp_zone_adj = 0.0
    zone_q = float(scalp_zone_ctx.get("zone_quality") or 0.0)
    if bool(scalp_zone_ctx.get("favorable")):
        scalp_zone_adj += 3.0 + 2.0 * zone_q
    if bool(scalp_zone_ctx.get("hostile")):
        scalp_zone_adj -= 4.0 + 3.0 * zone_q
...
def compute_ride_signal(
    ...
    **_ignored: object,
    ...
    ride_zone_adj = 0.0
    zone_q = float(ride_zone_ctx.get("zone_quality") or 0.0)
    if bool(ride_zone_ctx.get("favorable")):
        ride_zone_adj += 4.0 + 3.0 * zone_q
    if bool(ride_zone_ctx.get("hostile")):
        ride_zone_adj -= 6.0 + 4.0 * zone_q
        if str(entry_mode or '').upper() == 'BREAKOUT':
            ride_zone_adj -= (2.0 + 2.0 * zone_q)
